...
import { arrayNumberToUint8Array, stringToUint8Array, uint8ArrayToNumberArray, uint8ArrayToString } from "../utils";
...
export class ContractMethod implements ISerializable{
    ...
    public constructorOne(name: string, returnType: VMType, labels: Map<string, number>, parameters: ContractParameter[]) {
        if (!labels.has(name)) {
